...
import { Submenu } from '@/constants/admin';
  ...
  return isOpen ? (
    <Collapsible open={isCollapsed} onOpenChange={setIsCollapsed} className="w-full">
      <CollapsibleTrigger className="mb-1 [&[data-state=open]>div>div>svg]:rotate-180" asChild>
        <Button variant={active ? 'primary' : 'ghost'} className="h-10 w-full justify-start">
          <div className="flex w-full items-center justify-between">
            ...
            >
              <Icons.ChevronDown size={18} className="transition-transform duration-200" />
            </div>
          </div>
        </Button>
      </CollapsibleTrigger>
      ...
      >
        {submenus.map(({ href, label, active, icon }, index) => {
          // @ts-expect-error: Dynamic access to Icons based on icon
          const IconComponent = icon ? Icons[icon] : Icons.Tag;
          return (
            <Button
              key={index}
